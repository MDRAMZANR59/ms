...
<?php 
    ...
    $result=$mysqli->common_select_query("select supplier.name as sup_name,supplier.contact,purchase.* from purchase join supplier on supplier.id=purchase.supplier_id where purchase.id=$id");
    if($result){
        if($result['data']){
    ...
    $result=$mysqli->common_select_query("SELECT purchase_details.*, medicine.brand_name,medicine.generic_name FROM `purchase_details` JOIN medicine on medicine.id=purchase_details.medicine_id where purchase_details.purchase_id=$id");
    if($result){
        if($result['data']){
            foreach($result['data'] as $data){
                $medicines[]=$data;
          ...
          <div class="row">
            <div class="equalHWrap eqWrap">
              <div class="equalHW eq infoblock to-block">
                <span id="supplierName"><?= $invoice_data->sup_name ?></span><br>
              ...
              <div class="equalHW eq infoblock info-block">
                <span id="">Date:</span> <span class="right" id="CreatedDate"><?= date('d-m-Y',strtotime($invoice_data->purchase_date)); ?></span><br>
